...
for plot_id, df in data_by_plot.items():
    ...
    if coords:
        ...
        folium.map.Marker(
            ...
            icon=folium.DivIcon(
                html=f"""
                    <div style="font-size: 10px;font-weight: bold; color: white; background-color: purple; border-radius: 4px; padding: 3px 4px; text-align: left; transform: translate(-80%, -50%); position: relative; width: auto;">
                        {plot_id}
                    </div>
                """
            ),
